...
function validateDocument(document: CanvasDocumentV3): CanvasDocumentV3 {
  ...
  if (hashCanvasDocumentV3(snapshot) !== snapshot.stateHash) {
    throw new BunCanvasDocumentV3JournalConflictError(
...
export class BunCanvasDocumentV3JournalConflictError extends Error {
...
/**
 * Bun-native SQLite authority for a V3 document's verified snapshot and
 * append-only operation journal. This module must remain independent from the
 * Node SQLite implementation so Bun sidecars never load a Node SQLite module.
 */
export class BunSqliteCanvasDocumentV3PersistencePort
  implements CanvasDocumentV3PersistencePort
{
  ...
  constructor(databasePath: string) {
    ...
    try {
      ...
      const json = this.#database
        .query<{ readonly supported: number }>(
          "SELECT json_valid('{}') AS supported",
        )
        .get();
  ...
  #load(identity: CanvasDocumentIdentityV3): CanvasDocumentJournalV3 | null {
    const snapshotRow = this.#database
      ...
      .get(identity.projectId, identity.documentId);
    if (snapshotRow === null) {
      return null;
    }
    ...
    const rows = this.#database
      ...
      .all(identity.projectId, identity.documentId);

...
export const MAX_WORKSPACE_DOCUMENTATION_BYTES = 1_048_576;
...
function canonicalize(value: unknown): string {
  ...
  if (
    ...
    if (typeof value === "number" && !Number.isFinite(value)) {
      throw new TypeError("Workspace documentation numbers must be finite.");
    }
  ...
  if (typeof value !== "object") {
    throw new TypeError("Workspace documentation must contain JSON values.");
  }
...
export function browserCanonicalJson(value: unknown): string {
  ...
  if (bytes > MAX_WORKSPACE_DOCUMENTATION_BYTES) {
...
export function hashBrowserCanonicalValue(value: unknown): string {
  return `sha256:${sha256(
    new TextEncoder().encode(browserCanonicalJson(value)),
  )}`;
}
